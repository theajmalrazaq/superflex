...
function NavBar({ currentPage = '', onAttendanceLinkFound }) {
    ...
    useEffect(() => {
                        ...
                        try {
                            if (element.href && !element.href.includes('javascript:void')) {
                                ...
                                links.push({
                                    ...
                                    text: element.querySelector(".m-menu__link-text")?.innerHTML ||
        ...
        if (links.length > 0) {
            setMenuLinks(links);
            const attendanceLink = links.find(link => link.text.trim() === 'Attendance');
        ...
        // Extract username
        const usernameElement = document.querySelector(".m-topbar__username.m--hidden-tablet.m--hidden-mobile.m--padding-right-15");
        if (usernameElement) {
                            ...
                            <ul className="space-y-1.5">
                                {menuLinks.map((link, index) => {
                                    ...
                                    if (link.text.trim() === 'Home') {
                                        isActive = currentPage === '/' || currentPage.startsWith('/?dump=');
                                    } else {
                                        isActive = link.path && currentPage && currentPage.includes(link.path);
                                    }
                            ...
                            <div className="w-12 h-12 rounded-xl overflow-hidden border-2 border-white/10  flex-shrink-0">
                                <img
                                    ...
                                    onError={({ currentTarget }) => {
                                        ...
                                        currentTarget.src = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%236b7280'%3E%3Cpath d='M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z'/%3E%3C/svg%3E";
                                    }}
                                />
                            </div>
