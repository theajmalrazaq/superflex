...
import { parseProfile, parseAttendance, parseMarks, parseStudyPlan } from "../utils/dataParsers";
...
const AiDataPrepare = ({ rawLinks }) => {
  ...
  // Process raw links into a structured map
  useEffect(() => {
    ...
    rawLinks.forEach((l) => {
      ...
      if (text.includes("Marks") && !text.includes("PLO")) linkMap.marks = l.href;
      if (text.includes("Tentative Study Plan") || text.includes("Study Plan")) linkMap.studyPlan = l.href;
    });
    
    setProcessedLinks(linkMap);
  }, [rawLinks]);

  // Load initial state from localStorage (only if permitted)
  useEffect(() => {
    ...
    try {
      ...
      if (savedCtx) {
        ...
        window.superflex_ai_context = { ...window.superflex_ai_context, ...cleanedCtx };
  ...
  const updateContext = useCallback((newData) => {
    try {
      const existing = JSON.parse(localStorage.getItem("superflex_ai_context") || "{}");
      const merged = { 
          ...existing, 
          ...newData,
      ...
      // Broadcast update
      window.dispatchEvent(new CustomEvent("superflex-data-updated", { detail: merged }));
    } catch(e) { console.error("Update Context Error", e); }
  }, []);

  const syncAll = useCallback(async (force = false) => {
    const permission = localStorage.getItem("superflex_ai_data_permission");
    if (permission !== "granted") {
      console.log("AiDataPrepare: Sync blocked - No permission granted.");
      return;
    }
    ...
    if (processedLinks.profile) {
      fetchQueue.push(fetchDoc(processedLinks.profile).then(doc => {
        if (doc) {
          const profileData = parseProfile(doc);
          if (profileData) {
            if (profileData.studentName) newData.studentName = profileData.studentName;
            if (profileData.universityInfo) newData.universityInfo = profileData.universityInfo;
            if (profileData.profileSections) newData.profileSections = profileData.profileSections;
            hasUpdates = true;
          }
        }
      }));
    ...
    if (processedLinks.attendance) {
      fetchQueue.push(fetchDoc(processedLinks.attendance).then(doc => {
        if (doc) {
    ...
    if (processedLinks.marks) {
      fetchQueue.push(fetchDoc(processedLinks.marks).then(doc => {
        if (doc) {
    ...
    if (processedLinks.studyPlan) {
      fetchQueue.push(fetchDoc(processedLinks.studyPlan).then(doc => {
        if (doc) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("superflex-trigger-scan", handleScanRequest);
  }, [syncAll]);
  ...
  useEffect(() => {
    if (Object.keys(processedLinks).length > 0) {
      ...
      if (!lastScanned) {
        console.log("AiDataPrepare: Triggering initial scan after login...");
        syncAll(false);
      }
    }
  }, [processedLinks, syncAll]);

  // Listen for permission granted event to start sync immediately
  useEffect(() => {
    const handlePermissionChange = (e) => {
      if (e.detail === "granted") {
        console.log("AiDataPrepare: Permission granted, starting sync...");
        syncAll(false);
      } else if (e.detail === "denied") {
        console.log("AiDataPrepare: Permission denied, clearing context...");
        ...
        window.dispatchEvent(new CustomEvent("superflex-data-updated", { detail: {} }));
      }
    };
    window.addEventListener("superflex-permission-updated", handlePermissionChange);
    return () => window.removeEventListener("superflex-permission-updated", handlePermissionChange);
  }, [syncAll]);
