...
const ActionBtn = ({ onClick, primary, children, icon: Icon }) => (
  <button
    ...
    className={`w-full flex items-center justify-center gap-3 py-4 px-8 rounded-2xl font-bold transition-all duration-300 ${
      primary ? "bg-[#a098ff] hover:bg-[#8f86ff] !text-white" : "bg-white/5 hover:bg-white/10 !text-white border border-white/5"
    }`}
  >
...
const SessionExpirePage = () => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center p-6 space-y-8 animate-in fade-in duration-700 font-sans">
      ...
      <div className="fixed inset-0 pointer-events-none opacity-20" style={{ backgroundImage: `url("${chrome.runtime.getURL("public/overlay.png")}")` }} />
      ...
      <div className="relative z-10 text-center space-y-6 max-w-lg">
        ...
        <div className="space-y-4">
          <h1 className="text-4xl font-black !text-white">Session <span className="!text-rose-500">Expired</span></h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <ActionBtn onClick={() => window.location.href = "/Login"} primary icon={ArrowRight}>Log In Again</ActionBtn>
          <ActionBtn onClick={() => window.location.href = "/"} icon={Home}>Home</ActionBtn>
        </div>
      </div>
    </div>
  );
