...
function RetakeExamPage() {
    ...
    const applyCustomStyling = (element) => {
        ...
        if (portletHead) {
            ...
            const semesterForm = element.querySelector('form[action="/Student/RetakeRequest"]');
            if (semesterForm) {
                ...
                formClone.classList.add("!flex", "!items-center", "!gap-4", "ml-4", "flex-1");
                ...
                formSelects.forEach(select => {
                    ...
                    select.classList.remove("m-dropdown__toggle", "btn", "btn-brand", "dropdown-toggle");
                });
        ...
        if (retakeReasonSection) {
            ...
            const retakeReasonSelect = retakeReasonSection.querySelector("#DrpDwnRetakeReason");
            if (retakeReasonSelect) {
                ...
                // Remove bootstrap classes
                retakeReasonSelect.classList.remove("m-dropdown__toggle", "btn", "btn-brand", "dropdown-toggle");
                retakeReasonSelect.style.textAlign = "left";
            }

            // Find the buttons to move
            const retakeRequestBtn = element.querySelector("#btnModalExamRetakRequest");
            ...
            if (selectParent && retakeRequestBtn && downloadBtn) {
                ...
                `;
                retakeRequestBtn.classList.add("!w-10", "!h-10", "!p-0", "!justify-center");
                ...
                selectParent.replaceChild(selectWithButtonsContainer, retakeReasonSelect);
            }
        }
        ...
        buttons.forEach(button => {
            if (!button.closest(".modal-footer") && !button.classList.contains("close")) {
        ...
        modals.forEach(modal => {
            ...
            if (modalBody) {
                modalBody.classList.add("!bg-black", "!text-white", "!p-6");

                // Hide or remove the error alert div in the modal
                const errorDiv = modalBody.querySelector("#DataErrormsgdiv_RetakeModalReason");
                if (errorDiv) {
                ...
                fileInputs.forEach(input => {
                    ...
                    if (label && label.tagName === 'LABEL') {
                        label.classList.add("!text-white", "!font-medium", "!mb-2", "!block");
                    ...
                    container.className = "mt-1 flex justify-center px-6 pt-5 pb-6 !border !border-white/10 !rounded-xl bg-black/50 hover:bg-black/70 transition-colors cursor-pointer";
                    ...
                    newInput.className = "absolute inset-0 w-full h-full opacity-0 cursor-pointer";
            ...
            if (modalFooter) {
                ...
                const primaryBtn = modalFooter.querySelector('input[type="button"], .btn-primary');
                if (primaryBtn) {
        ...
        // Remove the checkbox update visual function since we're not using custom checkboxes anymore
    };
