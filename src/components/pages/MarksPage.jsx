                 ...
                 >
                    ...
                    {(course.grandTotalStats) && (
                        <span className={`text-xs font-bold px-2 py-1 rounded-lg ${
                            (course.grandTotalStats.percentage || 0) >= 80 ? "bg-emerald-500/20 text-emerald-400" :
                            (course.grandTotalStats.percentage || 0) >= 60 ? "bg-amber-500/20 text-amber-400" :
...
const AssessmentView = ({ section, onUpdate, markedItems, onToggleMark, courseId }) => {
  ...
  const statusColor = percentage >= 80 ? "text-emerald-400" :
                      percentage >= 60 ? "text-amber-400" :
                      "text-rose-400";
                      
  const statusBg = percentage >= 80 ? "bg-emerald-500/10" :
                   percentage >= 60 ? "bg-amber-500/10" :
                   "bg-rose-500/10";
  
  const borderColor = percentage >= 80 ? "border-emerald-500/20" :
                   percentage >= 60 ? "border-amber-500/20" :
                   "border-rose-500/20";

  return (
    <div className={`group rounded-[2rem] border transition-all duration-300 overflow-hidden ${
        isGrandTotal 
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 border-b border-white/5">
           ...
           <div className="flex items-center gap-4">
               <div className="flex flex-col items-end px-5 py-2.5 rounded-xl bg-black/20 border border-white/5 min-w-[120px]">
                   ...
                   <span className="text-lg font-bold text-white">{weight.toFixed(2)}</span>
               </div>
               <div className={`flex flex-col items-end px-5 py-2.5 rounded-xl border min-w-[120px] ${statusBg} ${borderColor}`}>
                   <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Obtained</span>
                ...
                <table className="w-full text-sm text-left border-collapse">
                    <thead>
                        <tr className="border-b border-white/10 bg-black/40 backdrop-blur-md">
                            <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-wider sticky top-0 bg-[#161616]/90">Item Name</th>
                            <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-wider text-center sticky top-0 bg-[#161616]/90">Weightage</th>
                            <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-wider text-center sticky top-0 bg-[#161616]/90">Total Marks</th>
                            <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-wider text-center sticky top-0 bg-[#161616]/90">Obtained</th>
                            <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-wider text-center sticky top-0 bg-[#161616]/90">Avg</th>
                            <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-wider text-center sticky top-0 bg-[#161616]/90">Std Dev</th>
                            <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-wider text-center sticky top-0 bg-[#161616]/90">Min</th>
                            <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-wider text-center sticky top-0 bg-[#161616]/90">Max</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                        {section.rows.filter(r => r.title !== "Total").map((row, idx) => {
                            ...
                            return (
                                ...
                                <td className="px-6 py-4 text-center text-zinc-500">~{row.avg.toFixed(2)}</td>
                                <td className="px-6 py-4 text-center text-zinc-500">~{row.stdDev.toFixed(2)}</td>
                    ...
                    <tfoot className="bg-[#1c1c1c]/50 border-t-2 border-white/10 font-bold backdrop-blur-sm">
                            ...
                            <td className="px-6 py-5 text-center text-white">{weight.toFixed(2)}</td>
                            <td className="px-6 py-5 text-center">-</td>
                            <td className="px-6 py-5 text-center text-[#a098ff] text-lg">{obtained.toFixed(2)}</td>
                            <td className="px-6 py-5 text-center text-zinc-400">~{section.stats.weightedAvg.toFixed(2)}</td>
                            <td className="px-6 py-5 text-center text-zinc-400">~{section.stats.weightedStdDev.toFixed(2)}</td>
                            <td className="px-6 py-5 text-center text-zinc-400 text-xs">~{section.stats.weightedMin.toFixed(2)}</td>
                            <td className="px-6 py-5 text-center text-zinc-400 text-xs">~{section.stats.weightedMax.toFixed(2)}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
...
const recalculateCourse = (course) => {
    ...
    const sectionsExceptTotal = course.sections.filter(s => s.title !== "Grand Total Marks");
      ...
      <div className="w-full min-h-screen p-4 md:p-8 space-y-8 animate-in fade-in duration-500 pb-24">
         ...
         {alertInfo && (
            <div className={`p-4 rounded-3xl border flex items-center gap-4 ${
                ...
                <div className={`p-2 rounded-full ${alertInfo.type === 'error' ? 'bg-rose-500/20' : 'bg-blue-500/20'}`}>
                    <AlertCircle size={20} />
                </div>
         ...
         {loading ? (
             <div className="flex flex-col items-center justify-center h-64 gap-4">
                ...
                <span className="text-zinc-500 text-sm font-bold animate-pulse">Loading Marks Data...</span>
             </div>
         ) : selectedCourse ? (
                 ...
                 <div className="flex flex-col gap-6">
                     ...
                     {activeSectionId && selectedCourse.sections.find(s => s.id === activeSectionId) ? (
                         ...
                         <div className="text-center py-24 bg-[#161616] rounded-[30px] border border-white/5 opacity-50">
                             <div className="text-xl font-bold text-zinc-500">No assessments found</div>
                         </div>
                     )}
                 </div>
