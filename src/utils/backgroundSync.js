...
export const runBackgroundSync = async () => {
  try {
    ...
    const marksRoot = marksDoc.querySelector(
    ...
    const attRoot = attDoc.querySelector(
      ".m-grid.m-grid--hor.m-grid--root.m-page",
    );
      ...
      c.sections.forEach((s) => {
        s.rows.forEach((r) => {
          ...
          if (!oldRow) {
            ...
            oldRow.obtained !== r.obtained ||
            oldRow.total !== r.total
          ) {
            notifications.unshift({
              id: Date.now() + Math.random(),
              title: "Marks Updated",
